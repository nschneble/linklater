...
const SCREENS_DIR = join('actions', 'screens');
...
export async function computeScreenCoverage(
  ...
  const screensRoot = join(rootDir, SCREENS_DIR);
  const baselinesRoot = join(rootDir, 'baselines');
  let entries: string[] = [];
  try {
  ...
  for (const name of screenNames) {
    const baseline = join(baselinesRoot, name, '0.png');
    try {
