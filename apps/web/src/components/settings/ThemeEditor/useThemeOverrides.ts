...
// re-exported so editor consumers import the token vocabulary from here
export {
  BUNDLES,
  CARD_BUNDLES,
  EDITABLE_VARS,
...
export const VAR_GROUPS: BundleGroup[] = BUNDLES.map((bundle) => ({
  ...
  items: [
    ...
    ...(bundle === 'base'
      ...
      : []),
    // focus ring lives in the base group so its contrast is checkable here
    ...(bundle === 'base'
...
export function useThemeOverrides(editorMode: Mode): UseThemeOverridesResult {
  ...
  const readBaseline = useCallback((): Record<ThemeVariable, string> => {
    ...
    // disabled: mirror via a probe, not :root, so tabs don't flip the site
  ...
  const loadOverrides = useCallback((tokens: Record<string, string>) => {
    // keys the copy omits keep their current value, never a stale pre-copy one
  ...
  // scoped subtree paints colorValues, never the global :root, in both modes
