...
// harness mirrors production nav driving the hook; its clear listeners live on
// document, so tests interact with real DOM nodes
...
function renderHarness(sectionIds: string[]) {
  for (const id of sectionIds) {
    ...
    element.id = id;
    // non-zero top so the scroll helper uses scrollIntoView, not the snap-to-0 path
...
beforeEach(() => {
  ...
  // jsdom puts focus on HTMLElement.prototype; mock there so the helper's focus is caught
...
afterEach(() => {
  // flush pending timers (e.g. the 5s safety-clear) inside act() to avoid act warnings
  act(() => {
