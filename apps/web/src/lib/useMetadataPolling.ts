...
/** How long to wait before making the first metadata poll after a link is saved (milliseconds). */
const INITIAL_INTERVAL_MS = 2_000;
/** Maximum back-off interval between polls (milliseconds). */
const MAX_INTERVAL_MS = 16_000;
/** Stop polling entirely after this many milliseconds have elapsed. */
const MAX_ELAPSED_MS = 60_000;

/**
 * Polls `GET /links/:id` with exponential back-off until the link's metadata
 * has been fetched (`link.meta.fetchedAt` is set), then calls `onSettled`.
 *
 * This hook exists because metadata is fetched asynchronously by a background
 * job after a link is saved. The UI shows a pulsing placeholder while
 ...
 * - Each subsequent poll: interval doubles (2s → 4s → 8s → 16s, then stays at 16s).
 * - Stops after 60 seconds total elapsed time, even if metadata never arrives.
 *
 * GOTCHA: `onSettled` is stored in a ref so the effect closure always calls
 * the latest version without needing `onSettled` in the dependency array.
 * Including `onSettled` directly would re-run the effect on every render.
 *
