...
- Slots: `bg`, `border`, `text`, `alt-text`, `highlight`. State bundles add `highlight-fg`, `highlight-hover`. Base adds `subtle-text`. Base+mount add `input-bg`.
- Shared components consume bundles via a `surface: 'base' | 'mount' | 'orbit'` prop (see `FormInput`, `SlidingTabBar`, `TabButton`). The host bundle is the rendering parent's surface, NOT the importing module's directory.
