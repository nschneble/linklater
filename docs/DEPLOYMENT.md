...
- **SSO callback re-registration.** OAuth callback URLs are pinned to localhost
  ...
  than broken. [GOOGLE-SSO.md](./GOOGLE-SSO.md) is the step-by-step for Google:
  the Cloud Console client, both redirect URIs, the API variables, and the
  build-time flag that renders the login button.
